...
def getWikiData(search_results):
    incode_url = quote(search_results)
    print("search : " , search_results)
    print("incode = ", incode_url)

    wiki_url = "https://ko.wikipedia.org/wiki/" + search_results
    headers = {'Content-Type': 'application/json'}
    response = requests.get(wiki_url)
    if response.status_code == 200:
        url = "https://ko.wikipedia.org/w/api.php?action=parse&parse&page=" + incode_url + "&prop=wikitext&formatversion=2&format=json"
...
def tokenizing(text):
    ...
    kiwi = Kiwi()
    kiwi.prepare()
    if 'parse' in text_data:
        print("======================================ok===============================")
        print(text_data['parse']['title'])
    else:
        print("errorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerrorerror")
        print(text_data)
        print(url)
...
def getWord(text):
    ...
    search_results = search_results[4950:]
    similar_word= []
    result ={}
    grouped_result = []
    word_group = {}
